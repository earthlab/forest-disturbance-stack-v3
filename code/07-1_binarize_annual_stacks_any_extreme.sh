...
for yr in {2000..2020}; do
  ...
  for MODE in any extreme; do
    ...
    # -----------------------------
    # Binarization with NoData preserved
    # -----------------------------

    # --- Wildfire (band 1)
    gdal_calc.py -A "${INFILE}" --A_band=1 \
      --calc="numpy.where(A == -9999, -9999, numpy.where(${WF_CALC}, 1, 0))" \
      --type=Int16 \
      --NoDataValue=-9999 \
    ...
    gdal_calc.py -A "${INFILE}" --A_band=2 \
      --calc="numpy.where(A == -9999, -9999, numpy.where(${BT_CALC}, 1, 0))" \
      --type=Int16 \
      --NoDataValue=-9999 \
      --co COMPRESS=DEFLATE --co TILED=YES \
      --outfile="${BT}"
    ...
    gdal_calc.py -A "${INFILE}" --A_band=3 \
      --calc="numpy.where(A == -9999, -9999, numpy.where(${HD_CALC}, 1, 0))" \
      --type=Int16 \
      --NoDataValue=-9999 \
      --co COMPRESS=DEFLATE --co TILED=YES \
      --outfile="${HD}"
    ...
    gdal_calc.py -A "${INFILE}" --A_band=4 \
      --calc="numpy.where(A == -9999, -9999, numpy.where(${PD_CALC}, 1, 0))" \
      --type=Int16 \
      --NoDataValue=-9999 \
      --co COMPRESS=DEFLATE --co TILED=YES \
      --outfile="${PD}"
...
echo "✅ ANY and EXTREME annual binary stacks created with correct NoData handling."
